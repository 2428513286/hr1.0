...
function deleteById(id,sp,qname,qmanager){
	if(confirm("确认要删除编号为【"+id+"】数据吗？")){
		location.href = "CertficateServlet?opType=deleteById&id="+id+"&currentPage="+sp+"&qname="+qname+"&qmanager="+qmanager;
	}
}
function deleteMore(sp,qname,qmanager){
	...
	if(confirm("您确定要删除这【"+num+"】条记录吗？")){
		...
		location.href = "CertficateServlet?opType=deleteMore&ids="+ids+"&currentPage="+sp+"&qname="+qname+"&qmanager="+qmanager;
	}
}
$(function(){
